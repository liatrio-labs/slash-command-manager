...
- [ ] 3.11 Run tests in Docker: `docker run --rm slash-man-test uv run pytest tests/integration/test_generate_command.py -v` and verify all tests pass
...
- [ ] 4.1 Create `tests/integration/test_github_integration.py` file
- [ ] 4.2 Write test `test_github_downloads_prompts_successfully()` that uses pytest-httpx to mock GitHub API response (directory with .md files), executes `slash-man generate --github-repo owner/repo --github-branch main --github-path prompts --agent claude-code --target-path <temp_dir> --yes`, verifies exit code is 0, and verifies files are generated with correct GitHub source metadata (source_type: "github", source_repo, source_branch, source_path)
- [ ] 4.3 Write test `test_github_single_file_download()` that uses pytest-httpx to mock GitHub API response (single file), executes `slash-man generate --github-repo owner/repo --github-branch main --github-path prompts/test-prompt-1.md --agent claude-code --target-path <temp_dir> --yes`, verifies exit code is 0, and verifies file is generated with correct source metadata
- [ ] 4.4 Write test `test_github_404_repository_error()` that uses pytest-httpx to mock 404 response, executes `slash-man generate --github-repo owner/repo --github-branch main --github-path prompts --agent claude-code --target-path <temp_dir> --yes`, verifies exit code is 3 (I/O error), and uses exact text matching to verify stderr contains expected error message about repository not found
- [ ] 4.5 Write test `test_github_404_branch_error()` that uses pytest-httpx to mock 404 response for invalid branch, executes `slash-man generate --github-repo owner/repo --github-branch invalid-branch --github-path prompts --agent claude-code --target-path <temp_dir> --yes`, verifies exit code is 3, and uses exact text matching to verify stderr contains expected error message
- [ ] 4.6 Write test `test_github_404_path_error()` that uses pytest-httpx to mock 404 response for invalid path, executes `slash-man generate --github-repo owner/repo --github-branch main --github-path invalid/path --agent claude-code --target-path <temp_dir> --yes`, verifies exit code is 3, and uses exact text matching to verify stderr contains expected error message
- [ ] 4.7 Write test `test_github_403_error()` that uses pytest-httpx to mock 403 response, executes `slash-man generate --github-repo owner/repo --github-branch main --github-path prompts --agent claude-code --target-path <temp_dir> --yes`, verifies exit code is 3, and uses exact text matching to verify stderr contains expected error message about access forbidden
- [ ] 4.8 Write test `test_github_network_error()` that uses pytest-httpx to mock network error (RequestException), executes `slash-man generate --github-repo owner/repo --github-branch main --github-path prompts --agent claude-code --target-path <temp_dir> --yes`, verifies exit code is 3, and uses exact text matching to verify stderr contains expected error message about network error
- [ ] 4.9 Write test `test_github_invalid_repo_format()` that executes `slash-man generate --github-repo invalid-format --github-branch main --github-path prompts --agent claude-code --target-path <temp_dir> --yes`, verifies exit code is 2 (validation error), and uses exact text matching to verify stderr contains expected validation error message
- [ ] 4.10 Write test `test_github_missing_required_flags()` that executes `slash-man generate --github-repo owner/repo --github-branch main --agent claude-code --target-path <temp_dir> --yes` (missing --github-path), verifies exit code is 2, and uses exact text matching to verify stderr contains expected error message about missing required flags
- [ ] 4.11 Write test `test_github_mock_uses_test_fixtures()` that verifies mocked GitHub API responses use content from `tests/integration/fixtures/prompts/` test files (not dynamically generated content)
- [ ] 4.12 Run tests in Docker: `docker run --rm slash-man-test uv run pytest tests/integration/test_github_integration.py -v` and verify all tests pass

### [ ] 5.0 File System Verification and Error Scenario Tests
...
- [ ] 5.1 Create `tests/integration/test_filesystem_and_errors.py` file
- [ ] 5.2 Write test `test_file_timestamps_set_correctly()` that generates a file, uses `pathlib.Path.stat()` to get file timestamp, and verifies timestamp is recent (within last minute)
- [ ] 5.3 Write test `test_file_content_structure_validation()` that generates a file, reads content, parses frontmatter (YAML), and verifies required metadata fields exist (name, description, source_type, etc.)
- [ ] 5.4 Write test `test_exact_file_content_comparison()` that generates a file, reads entire content, and compares with expected content string using exact text matching (assert content == expected_content)
- [ ] 5.5 Write test `test_invalid_flag_combination_error()` that executes `slash-man generate --prompts-dir <dir> --github-repo owner/repo --github-branch main --github-path prompts --agent claude-code --target-path <temp_dir> --yes` (mutually exclusive flags), verifies exit code is 2, and uses exact text matching to verify stderr contains expected error message about mutual exclusivity
- [ ] 5.6 Write test `test_missing_required_flags_error()` that executes `slash-man generate --agent claude-code --target-path <temp_dir> --yes` (no prompts-dir or GitHub flags), verifies exit code is 3, and uses exact text matching to verify stderr contains expected error message
- [ ] 5.7 Write test `test_invalid_agent_key_error()` that executes `slash-man generate --prompts-dir <fixtures_dir> --agent invalid-agent --target-path <temp_dir> --yes`, verifies exit code is 2, and uses exact text matching to verify stderr contains expected error message about invalid agent key
- [ ] 5.8 Write test `test_permission_denied_error()` that creates a read-only directory, executes `slash-man generate --prompts-dir <fixtures_dir> --agent claude-code --target-path <readonly_dir> --yes`, verifies exit code is 3, and uses exact text matching to verify stderr contains expected error message about permission denied
- [ ] 5.9 Create `tests/integration/test_cleanup_command.py` file
- [ ] 5.10 Write test `test_cleanup_dry_run_mode()` that generates files, executes `slash-man cleanup --agent claude-code --target-path <temp_dir> --dry-run`, verifies exit code is 0, verifies stdout shows files that would be deleted, and verifies files still exist
- [ ] 5.11 Write test `test_cleanup_removes_generated_files()` that generates files, executes `slash-man cleanup --agent claude-code --target-path <temp_dir> --yes`, verifies exit code is 0, and verifies files are deleted
- [ ] 5.12 Write test `test_cleanup_includes_backups()` that generates files and backups, executes `slash-man cleanup --agent claude-code --target-path <temp_dir> --include-backups --yes`, verifies exit code is 0, and verifies both command files and backup files are deleted
- [ ] 5.13 Write test `test_cleanup_excludes_backups_by_default()` that generates files and backups, executes `slash-man cleanup --agent claude-code --target-path <temp_dir> --yes`, verifies exit code is 0, and verifies backup files still exist
- [ ] 5.14 Write test `test_cleanup_multiple_agents()` that generates files for multiple agents, executes `slash-man cleanup --agent claude-code --agent cursor --target-path <temp_dir> --yes`, verifies exit code is 0, and verifies files for both agents are deleted
- [ ] 5.15 Write test `test_cleanup_all_agents()` that generates files for multiple agents, executes `slash-man cleanup --target-path <temp_dir> --yes` (no --agent flag), verifies exit code is 0, and verifies files for all agents are deleted
- [ ] 5.16 Run tests in Docker: `docker run --rm slash-man-test uv run pytest tests/integration/test_filesystem_and_errors.py tests/integration/test_cleanup_command.py -v` and verify all tests pass
- [ ] 5.17 Add integration test job to `.github/workflows/ci.yml` that runs `docker build -t slash-man-test .` and `docker run --rm slash-man-test uv run pytest tests/integration/ -v` on every PR (not optional, fails CI on failure)
- [ ] 5.18 Add integration test hook to `.pre-commit-config.yaml` that runs `docker build -t slash-man-test .` and `docker run --rm slash-man-test uv run pytest tests/integration/ -v` on push (stages: [pre-push], always_run: true)
- [ ] 5.19 Run full integration test suite in Docker: `docker run --rm slash-man-test uv run pytest tests/integration/ -v` and verify all tests pass
- [ ] 5.20 Verify CI integration: Push changes and verify integration tests run automatically in GitHub Actions
